...
public:
    ...
    virtual std::unique_ptr<ESP_SSLClient> create()
    {
        return std::unique_ptr<ESP_SSLClient>(new ESP_SSLClient());
    }

    virtual bool verify(ESP_SSLClient &client, const char *host)
    {
        return true;
    }
};
...
public:
    ...
    std::unique_ptr<ESP_SSLClient> create() override
    {
        return std::unique_ptr<ESP_SSLClient>(new ESP_SSLClient());
    }

    bool verify(ESP_SSLClient &client, const char *host) override
    {
        ESP_SSLClient &wcs = static_cast<ESP_SSLClient &>(client);
        if (_cacert == nullptr)
        {
            wcs.setInsecure();
        }
 ...
 * @return success bool
 */
bool HTTPClient::begin(ESP_SSLClient &client, String url)
{
#ifdef HTTPCLIENT_1_1_COMPATIBLE
    if (_tcpDeprecated)
    {
 ...
 * @return success bool
 */
bool HTTPClient::begin(ESP_SSLClient &client, String host, uint16_t port, String uri, bool https)
{
#ifdef HTTPCLIENT_1_1_COMPATIBLE
    if (_tcpDeprecated)
    {
 ...
 */
ESP_SSLClient &HTTPClient::getStream(void)
{
    if (connected())
    {
        return *_client;
 ...
 * @return NetworkClient*
 */
ESP_SSLClient *HTTPClient::getStreamPtr(void)
{
    if (connected())
    {
        return _client;
...
void HTTPClient::addHeader(const String &name, const String &value, bool first, bool replace)
{
    ...
    if (!name.equalsIgnoreCase(F("Connection")) && !name.equalsIgnoreCase(F("User-Agent")) && !name.equalsIgnoreCase(F("Accept-Encoding")) && !name.equalsIgnoreCase(F("Host")) && !(name.equalsIgnoreCase(F("Authorization")) && _base64Authorization.length()))
    {
